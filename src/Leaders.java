...
public class Leaders { // класс дл€ обработки лучших результатов

	private static Leaders leads;
	private String path; // путь к файлу
	private String highScores;
	
	// храним счет за все врем€ 
	...
	public static Leaders getInstance() { // добавл€ем экземпл€р, если его нет
		if(leads == null){
			leads = new Leaders();
		}
		return leads;
	...
	public void addScore(int score) { // добавл€ем лучший счет
	...
	public void addCube(int cubeValue) { // добавл€ем лучший куб
	...
	private void formSaveData() { // формируем данные
		try {
			...
			writer.write("0-0-0-0-0"); // заполн€м нул€ми
			writer.newLine();
	...
	public void saveScores() { // сохран€ем счет из игры
		FileWriter output = null;
		try {
			File file = new File(path, highScores);
			output = new FileWriter(file);
			BufferedWriter writer = new BufferedWriter(output);
			// добавл€ем новый результат между лучшим и худшим результатом, раздел€€ минусами
